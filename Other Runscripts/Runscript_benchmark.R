...
cpuvector <- c("%CPU %MEM     PID CMD ")
cpuvector <- append(cpuvector,system("ps -C rsession -o %cpu,%mem,pid,cmd | grep 'gebruiker'", intern = TRUE))
print("results point")
...
PMs = getBM(attributes = c("minor_allele_freq","chromosome_start"),
            filters = "ensembl_gene_id",
            values = ensembl_ID,
            mart = martHS) %>%
  as_tibble() %>%
...
########kijken naar de range> chr_start moet vervangen worden met gehele range van sequence en opgeteld worden
...
#Rprofmem(NULL)
# Analyze the output
#summaryRprof("Rprofmem.out", memory="both")
print(cpuvector)
